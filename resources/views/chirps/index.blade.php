<x-app-layout>
    ...
    <div class="max-w-2xl mx-auto p-4 sm:p-6 lg:p-8 ">
        <form action=" {{ route('chirps.store') }} " method="post">
            ...
            focus:ring-opacity-50 rounded-md shadow-sm"></textarea>
            <x-input-error :messages="$errors->get('message')" class="mt-2" />
            <x-primary-button class="mt-4"> {{ __('util.chirp') }}
            </x-primary-button>
        </form>
                    ...
                    <div class="flex-1">
                        <div class="flex justify-between items-center">
                            ...
                            @if ($chirp->user->is(auth()->user()))
                                {{-- Auto::user()  vérifie aussi l'utisateur correspond --}}
                                <x-dropdown>
                                    <x-slot name="trigger">
                                        <button>
                                            <svg class="text-red-600" xmlns="http://www.w3.org/2000/svg" width="16"
